...
import xpertss.crypto.x509.Extension;

import static java.security.KeyStore.*;
import static xpertss.crypto.KeyTool.Command.*;
import static xpertss.crypto.KeyTool.Option.*;
...
public final class KeyTool {
   ...
   enum Command {
      CERTREQ("Generates.a.certificate.request",
         ALIAS, SIGALG, FILEOUT, KEYPASS, KEYSTORE, DNAME, STOREPASS, STORETYPE,
         V, EXT, PROTECTED),
      CHANGEALIAS("Changes.an.entry.s.alias",
         ALIAS, DESTALIAS, KEYPASS, KEYSTORE, STOREPASS, STORETYPE, V, PROTECTED),
      DELETE("Deletes.an.entry",
         ALIAS, KEYSTORE, STOREPASS, STORETYPE, V, PROTECTED),
      EXPORTCERT("Exports.certificate",
         RFC, ALIAS, FILEOUT, KEYSTORE, STOREPASS, STORETYPE, V, PROTECTED),

      EXPORTCHAIN("Exports.certchain",
         RFC, ALIAS, FILEOUT, KEYSTORE, STOREPASS, STORETYPE, V, PROTECTED),
      EXPORTPEM("Exports.pemchain",
         ALIAS, FILEOUT, KEYSTORE, STOREPASS, STORETYPE, V, PROTECTED),
      EXPORTPRIKEY("Exports.private",
         RFC, ALIAS, FILEOUT, KEYSTORE, KEYPASS, STOREPASS, STORETYPE, V,
         PROTECTED),

      GENKEYPAIR("Generates.a.key.pair",
         ALIAS, KEYALG, KEYSIZE, SIGALG, DESTALIAS, DNAME, STARTDATE, EXT,
         VALIDITY, KEYPASS, KEYSTORE, STOREPASS, STORETYPE, V, PROTECTED),
      GENSECKEY("Generates.a.secret.key",
         ALIAS, KEYPASS, KEYALG, KEYSIZE, KEYSTORE, STOREPASS, STORETYPE,
         V, PROTECTED),
      GENCERT("Generates.certificate.from.a.certificate.request",
         RFC, INFILE, OUTFILE, ALIAS, SIGALG, DNAME, STARTDATE, EXT,
         VALIDITY, KEYPASS, KEYSTORE, STOREPASS, STORETYPE, V, PROTECTED),
      IMPORTCERT("Imports.a.certificate.or.a.certificate.chain",
         NOPROMPT, TRUSTCACERTS, PROTECTED, ALIAS, FILEIN, KEYPASS,
         KEYSTORE, STOREPASS, STORETYPE, V),
      IMPORTKEYSTORE("Imports.one.or.all.entries.from.another.keystore",
         SRCKEYSTORE, DESTKEYSTORE, SRCSTORETYPE, DESTSTORETYPE,
         SRCSTOREPASS, DESTSTOREPASS, SRCPROTECTED, SRCALIAS, DESTALIAS,
         SRCKEYPASS, DESTKEYPASS, NOPROMPT, V),
      KEYPASSWD("Changes.the.key.password.of.an.entry",
         ALIAS, KEYPASS, NEW, KEYSTORE, STOREPASS, STORETYPE, V),
      LIST("Lists.entries.in.a.keystore",
         RFC, ALIAS, KEYSTORE, STOREPASS, STORETYPE, V, PROTECTED),
      ...
      STOREPASSWD("Changes.the.store.password.of.a.keystore",
         NEW, KEYSTORE, STOREPASS, STORETYPE, V);
   ...
   private static final java.util.ResourceBundle rb =
      java.util.ResourceBundle.getBundle("xpertss.crypto.i18n.Resources");
   ...
   void doCommands(PrintStream out) throws Exception
   {
      ...
      // If the command is "genkey", "import", or "printcert",
      ...
      if (isKeyStoreRelated(command)) {
         ...
         if (!nullStream) {
            try {
               ...
               if (command != GENKEYPAIR && command != GENSECKEY && command != IMPORTCERT
                     && command != IMPORTKEYSTORE && command != PRINTCRL) {
      ...
      if ((command == CHANGEALIAS) && dest == null) {
      ...
      if (nullStream && storePass != null) {
         ...
         if (!protectedPath && !KeyStoreUtil.isWindowsKeyStore(storetype) &&
            (command == CERTREQ || command == DELETE || command == GENKEYPAIR ||
               command == GENSECKEY || command == IMPORTCERT || command == IMPORTKEYSTORE ||
               command == CHANGEALIAS || command == STOREPASSWD || command == KEYPASSWD)) {
            int count = 0;
            do {
         ...
         } else if (!protectedPath && isKeyStoreRelated(command)
      ...
      if (command == PRINTCERT || command == IMPORTCERT || command == PRINTCRL) {
      ...
      if (command == CERTREQ) {
         ...
         if (filename != null) {
            MessageFormat form = new MessageFormat(rb.getString("Certchain.stored.in.file.filename."));
            Object[] source = {filename};
            System.err.println(form.format(source));
         }
      } else if (command == EXPORTPEM) {
         PrintStream ps = null;
         if (filename != null) {
            ps = new PrintStream(new FileOutputStream(filename));
            out = ps;
         ...
         if (filename != null) {
            MessageFormat form = new MessageFormat(rb.getString("Private.stored.in.file.filename."));
            Object[] source = {filename};
            System.err.println(form.format(source));
         }
      ...
      if (kssave) {
         ...
         if (token) {
            ...
            try {
               fout = (nullStream ? null : new FileOutputStream(ksfname));
      ...
      throws Exception
   {
      ...
      X509CertImpl signerCertImpl = X509CertImpl.toImpl(signerCert);
      ...
      // NOTE New form is we arrange certs from root to user and we store in pkcs7
      // format similar to how we get software publisher certificates (spc files)
      List<X509Certificate> certs = new ArrayList<>();
      Certificate[] ca_certs = keyStore.getCertificateChain(alias);
      for(int i = ca_certs.length - 1; i >= 0; i--) {
         certs.add((X509Certificate) ca_certs[i]);
      }
      certs.add(cert);
      ...
      throws Exception
   {
      ...
      // START INCLUDE v3 EXTENSIONS IF PRESENT
      X509CertImpl impl = X509CertImpl.toImpl(cert);
      ...
      // This one definitely belongs to the signer so remove if present
      exts.deleteIfPresent(IssuerAlternativeNameExtension.NAME);
      // END INCLUDE v3 EXTENSIONS IF PRESENT

      exts = createV3Extensions(null, exts, v3ext, cert.getPublicKey(), null);
      // Attribute name is not significant
      request.getAttributes().setAttribute(X509CertInfo.EXTENSIONS,
         new PKCS10Attribute(PKCS9Attribute.EXTENSION_REQUEST_OID, exts));
    ...
    * Exports a certificate from the keystore.
    */
   private void doExportCertChain(String alias, PrintStream out)
      throws Exception
   {
      if (storePass == null && !KeyStoreUtil.isWindowsKeyStore(storetype)) {
         printWarning();
      ...
      // getCertificateChain returns the certs from user to root order
      Certificate[] certs = keyStore.getCertificateChain(alias);
      if (certs == null || certs.length < 2) {
         MessageFormat form = new MessageFormat(rb.getString("Alias.alias.has.no.certchain"));
         Object[] source = {alias};
         throw new Exception(form.format(source));
      }
    ...
    * Exports a certificate from the keystore.
    */
   private void doExportPEM(String alias, PrintStream out)
      throws Exception
   {
      if (storePass == null && !KeyStoreUtil.isWindowsKeyStore(storetype)) {
         printWarning();
    ...
    * Exports a certificate from the keystore.
    */
   private void doExportPrivateKey(String alias, PrintStream out)
      throws Exception
   {
      if (storePass == null && !KeyStoreUtil.isWindowsKeyStore(storetype)) {
         printWarning();
      ...
      Pair<Key, char[]> objs = recoverKey(alias, storePass, keyPass);
      PrivateKey priKey = (PrivateKey) objs.fst;
      if (keyPass == null) {
         keyPass = objs.snd;
      }

      dumpPrivateKey(priKey, out);
   }
    ...
    */
   private void doGenSecretKey(String alias, String keyAlgName, int keysize)
      throws Exception
   {
      if (alias == null) {
         alias = keyAlias;
      ...
      throws Exception
   {
      ...
      if (verbose) {
         ...
         Object[] source = {
            new Integer(keysize), privKey.getAlgorithm(), chain[0].getSigAlgName(), new Long(validity), x500Name
         };
         System.err.println(form.format(source));
      }
    ...
    */
   private void doPrintEntry(String alias, PrintStream out, boolean printWarning)
      throws Exception
   {
      ...
      if (keyStore.entryInstanceOf(alias, KeyStore.SecretKeyEntry.class)) {
         ...
         if (chain != null) {
            ...
            if (verbose || rfc || debug) {
               ...
               out.println(rb.getString("Certificate.fingerprint.SHA1.") +
                              getCertFingerPrint("SHA1", chain[0].getEncoded()));
            }
         }
         ...
         if (verbose && (cert instanceof X509Certificate)) {
            ...
            out.println(rb.getString("Certificate.fingerprint.SHA1.") + getCertFingerPrint("SHA1", cert.getEncoded()));
         }
      } else {
      ...
      throws Exception
   {
      ...
      out.println(rb.getString("Public.key.fingerprint.SHA1.") +
         getCertFingerPrint("SHA1", pkey.getEncoded()));
      ...
      throws Exception
   {
      ...
      X509Certificate oldCert = (X509Certificate) keyStore.getCertificate(alias);
      if (oldCert == null) {
      ...
      X509CertImpl certImpl = X509CertImpl.toImpl(oldCert);
      ...
      throws Exception
   {
      ...
      Object[] source = {cert.getSubjectDN().toString(),
                           ...
                           getCertFingerPrint("MD5", cert.getEncoded()),
                           getCertFingerPrint("SHA1", cert.getEncoded()),
                           getCertFingerPrint("SHA-256", cert.getEncoded()),
                           cert.getSigAlgName(),
                           cert.getVersion()
      };
      out.println(form.format(source));
      ...
      while (i1.hasNext() || i2.hasNext()) {
         ...
         if (ext.getClass() == Extension.class) {
            ...
            if (v.length == 0) {
               ...
               System.out.println(HexUtils.toHexString(ext.getExtensionValue()));
   ...
   private X500Name getX500Name() throws IOException
   {
      ...
      do {
         ...
         country = inputString(in,
            rb.getString("What.is.the.two.letter.country.code.for.this.unit."),
            country);
   ...
   /**
    * Writes a PKCS7 encoded certificate chain in base64 or binary encoding to
    * an output stream.
    */
   private void dumpChain(PKCS7 pkcs, PrintStream out)
      throws IOException, CertificateException
   {
      ByteArrayOutputStream baos = new ByteArrayOutputStream();
      pkcs.encodeSignedData(baos);
      if (rfc) {
         BASE64Encoder encoder = new BASE64Encoder();
         out.println("-----BEGIN PKCS7-----");
         encoder.encodeBuffer(baos.toByteArray(), out);
         out.println("-----END PKCS7-----");
      } else {
         out.write(baos.toByteArray()); // binary
      }
   }

   /**
    * Writes a collection of X.509 certificates to the specified output
    * stream, individually PEM encoding each.
    *
    * <pre>
    *   -----BEGIN CERTIFICATE-----
    *   Optional: User certificate
    *   -----END CERTIFICATE-----
    *   -----BEGIN CERTIFICATE-----
    ...
    * </pre>
    */
   private void dumpChain(Certificate[] chain, PrintStream out)
      throws IOException, CertificateException
   {
      BASE64Encoder encoder = new BASE64Encoder();
   ...
   /**
    * Writes a PrivateKey in base64 or binary encoding to an output
    * stream.
    * <p/>
    * PEM headers
    * http://stackoverflow.com/questions/5355046/where-is-the-pem-file-format-specified
    */
   private void dumpPrivateKey(PrivateKey priKey, PrintStream out)
      throws IOException, KeyException
   {
      if (rfc) {
         BASE64Encoder encoder = new BASE64Encoder();
         String algName = priKey.getAlgorithm();
         out.println(
            MessageFormat.format(
               "-----BEGIN {0} PRIVATE KEY-----",
               algName));
         encoder.encodeBuffer(priKey.getEncoded(), out);
         out.println(
            MessageFormat.format(
               "-----END {0} PRIVATE KEY-----",
               algName));
      } else {
         out.write(priKey.getEncoded()); // binary
      }
   }
    ...
    */
   private Pair<Entry, char[]> recoverEntry(KeyStore ks, String alias,
                                            char[] pstore, char[] pkey)
      throws Exception
   {
    ...
    */
   private String getCertFingerPrint(String mdAlg, byte[] data)
      throws Exception
   {
      ...
      throws Exception
   {
      ...
      Hashtable<Principal, List<Certificate>> certs = null;
      if (keyStore.size() > 0) {
         certs = new Hashtable<Principal, List<Certificate>>(11);
      ...
      List<Certificate> chain = new ArrayList<>(2);
   ...
   private boolean buildChain(X509Certificate certToVerify,
                              ...
                              Hashtable<Principal, List<Certificate>> certs)
   {
      Principal issuer = certToVerify.getIssuerDN();
      if (isSelfSigned(certToVerify)) {
         // reached self-signed root cert;
         // no verification needed because it's trusted.
         chain.add(certToVerify);
         return true;
      }

      // Get the issuer's certificate(s)
      List<Certificate> vec = certs.get(issuer);
      if (vec == null) {
         return false;
      }
    ...
    * All certs belonging to the same CA are stored in a list that
    * in turn is stored in the hashtable, keyed by the CA's subject DN
    */
   private void keystorecerts2Hashtable(KeyStore ks, Hashtable<Principal, List<Certificate>> hash)
      throws Exception
   {
      ...
      for (Enumeration<String> aliases = ks.aliases(); aliases.hasMoreElements(); ) {
         ...
         if (cert != null) {
            ...
            List<Certificate> vec = hash.get(subjectDN);
            if (vec == null) {
               vec = new ArrayList<>();
               vec.add(cert);
            } else {
      ...
      try {
         ...
         for (String extstr : extstrs) {
            ...
            switch (exttype) {
               ...
               case 2:     // EKU
                  if (value != null) {
                     List<ObjectIdentifier> v = new ArrayList<>();
                     for (String s : value.split(",")) {
                        int p = oneOf(s,
                           "anyExtendedKeyUsage",
                           "serverAuth",       //1
